    ...
    <div className="bg-white min-h-screen">
      ...
      <header className="bg-primary-800 text-white pt-16 pb-12">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold leading-tight mb-4 max-w-3xl">
            {post.title}
          </h1>
          <p className="text-primary-200 text-lg max-w-3xl mb-6">{post.excerpt}</p>
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
          ...
          <article className="min-w-0 max-w-3xl">
            ...
            <div className="mt-8 pt-8 border-t border-gray-200">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(`https://mygeocheck.com/blog/${post.slug}`);
                    const btn = document.getElementById('copy-link-btn');
                    if (btn) {
                      const orig = btn.innerHTML;
                      btn.innerHTML = '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg> Link Copied!';
                      btn.classList.add('bg-green-100', 'text-green-700');
                      setTimeout(() => { btn.innerHTML = orig; btn.classList.remove('bg-green-100', 'text-green-700'); }, 2000);
                    }
                  }}
                  id="copy-link-btn"
                  className="flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2.5 rounded-lg transition-colors text-sm font-medium"
            ...
            {/* CTA */}
            <div className="mt-8 bg-gradient-to-r from-accent-500 to-accent-600 rounded-xl p-8 text-center text-white">
        ...
        {relatedPosts.length > 0 && (
          <section className="mt-16 max-w-3xl">
            <h2 className="text-2xl font-bold text-primary-800 mb-6">Related articles</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {relatedPosts.map((rp) => (
                <Link
                  key={rp.slug}
                  href={`/blog/${rp.slug}`}
                  className="group block bg-white border border-gray-200 rounded-xl p-6 hover:shadow-md hover:border-primary-200 transition-all"
                >
                  ...
                  <h3 className="font-bold text-gray-900 group-hover:text-accent-600 transition-colors leading-snug mb-2">
                    {rp.title}
                  </h3>
