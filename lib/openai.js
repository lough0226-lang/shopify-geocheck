...
async function callMoonshotAPI(messages, model = 'qwen3.7-plus') {
  ...
  console.log(`[Bailian] model=${model}, base_url=${baseUrl}, max_tokens=8000`);
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      throw new Error('Bailian API request timed out after 60 seconds');
    }
    throw error;
  }
}
...
export async function analyzeProduct(productContent, productUrl, lang = 'en') {
  ...
  const langInstruction = lang === 'en'
    ? ''
    : `\n7. Output all text in ${langName}. Keep technical terms (GEO, SEO, schema markup, structured data, etc.) in English.`;
...
6. Your response must contain valid JSON only${langInstruction}`;
  ...
  const DELAYS = [3000, 6000]; // 指数退避：3秒、6秒
