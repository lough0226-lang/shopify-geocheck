  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (content && content.trim().length > 0) {
        ...
        // 验证价格相关诊断：如果 AI 声称价格为$0.00 但实际数据有价格，删除该诊断
        const actualPrice = productContent.price || '';
        const hasVariants = productContent.variants && productContent.variants.length > 0;
        // Shopify 产品页在未选择规格前显示$0.00 是正常行为，不是问题
        const isNormalZeroPrice = hasVariants || (actualPrice && actualPrice !== '$0.00' && actualPrice !== '0');
        if (isNormalZeroPrice) {
          normalized.diagnosis = (normalized.diagnosis || []).filter(d => {
            ...
            const isFalsePriceClaim = issue.includes('$0.00') || 
              ...
              issue.includes('price.*0\\.00') ||
              issue.includes('unclear') && issue.includes('price') ||
              issue.includes('confusing') && issue.includes('price') ||
              category.includes('price') && issue.includes('0.00');
