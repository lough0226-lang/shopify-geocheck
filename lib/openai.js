  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (content && content.trim().length > 0) {
        ...
        // 5 个位置固定类型：0=use-case, 1=product-fit, 2=comparison, 3=trust, 4=price
        ...
        if (allergenIndices.length > 1) {
          ...
          const posReplacements = {
            ...
            2: `What makes ${normalized.product_name} stand out from competitors?`, // comparison (no "vs" to avoid overlap)
            3: `Do customers enjoy ${normalized.product_name} for repeat purchases?`, // trust
            4: `What value do you get with ${normalized.product_name}?` // price/value
          };
