...
function normalizeResult(raw) {
  ...
  // 确保 buyer_queries 是 7 个
  let buyerQueries = Array.isArray(raw.buyer_queries) ? raw.buyer_queries : [];
  while (buyerQueries.length < 7) buyerQueries.push('best product in this category');
  buyerQueries = buyerQueries.slice(0, 7);

  // 确保 query_match_scores 对应 7 个
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (content && content.trim().length > 0) {
        ...
        // 强制买家查询多样性：7 个查询最多 1 个涉及过敏/饮食限制
        // 7 个位置固定类型：0=use-case, 1=product-fit, 2=comparison, 3=gift/occasion, 4=alternatives, 5=trust, 6=price/value
        // 替换时必须用与该位置类型匹配的模板，避免与同类型查询重复
        ...
        if (allergenIndices.length > 1) {
          // 每个位置的替换模板严格匹配该位置的类型，不使用 "compared to" 等易混淆句式
          const posReplacements = {
            0: `What are the best [category] products for everyday use?`, // use-case
            1: `Does ${normalized.product_name} come in different sizes or varieties?`, // product-fit
            2: `What makes ${normalized.product_name} stand out from the crowd?`, // comparison (no brand names)
            3: `What's a good ${normalized.product_name} for gifting?`, // gift/occasion
            4: `Are there simpler or more affordable options like ${normalized.product_name}?`, // alternatives
            5: `Do customers enjoy ${normalized.product_name} for repeat purchases?`, // trust
            6: `What value do you get with ${normalized.product_name}?` // price/value
          };
          for (let i = 1; i < allergenIndices.length; i++) {
            const pos = allergenIndices[i];
            bq[pos] = posReplacements[pos] || `Tell me more about ${normalized.product_name}`;
          }
          normalized.buyer_queries = bq;
          console.log(`[analyze] Enforced query diversity: replaced ${allergenIndices.length - 1} duplicate allergen queries at positions ${allergenIndices.slice(1).join(',')}`);
        }
        ...
        (normalized.query_match_scores || []).forEach(q => {
          ...
          // 修复：非过敏查询的 match score 理由中不应出现过敏相关词汇
          const queryLower = (q.query || '').toLowerCase();
          const reasonLower = (q.reason || '').toLowerCase();
          const queryIsAllergenRelated = allergenKeywords.some(k => queryLower.includes(k));
          const reasonHasAllergen = reasonLower.includes('allergen') || reasonLower.includes('allergy');
          if (!queryIsAllergenRelated && reasonHasAllergen) {
            // 理由提到了过敏但查询本身与过敏无关，替换为通用理由
            if (q.match === 'fail') {
              q.reason = 'Page does not provide specific information related to this query';
            } else if (q.match === 'low') {
              q.reason = 'Page has some relevant information but does not directly address this query';
            }
            console.log(`[analyze] Fixed allergen mismatch in reason for: "${q.query}"`);
          }
        });
        // 计算 AI 曝光率（exposure_score）：medium/high 匹配数 ÷ 总查询数 × 100
        const totalQueries = (normalized.query_match_scores || []).length;
        if (totalQueries > 0 && normalized.industry_benchmark) {
          const goodMatches = (normalized.query_match_scores || []).filter(q => q.match === 'medium' || q.match === 'high').length;
          normalized.industry_benchmark.exposure_score = Math.round(goodMatches / totalQueries * 100);
          console.log(`[analyze] exposure_score: ${normalized.industry_benchmark.exposure_score}% (${goodMatches}/${totalQueries} queries match medium+)`);
        }
...
/**
 * 复查函数 — 用原报告锁定的买家查询重新评分，不重新生成查询
 * 返回 { score, query_match_scores, exposure_score }
 */
export async function recheckProduct(productContent, originalQueries, lang = 'en') {
  const { buildRecheckPrompt } = await import('./prompts.js');
  const prompt = buildRecheckPrompt(productContent, originalQueries, lang);

  const systemPrompt = `You are a GEO (Generative Engine Optimization) expert performing a follow-up recheck.
Respond with ONLY valid JSON starting with { and ending with }.
Use double quotes for all keys and string values. No markdown, no code fences, no comments.
Return the SAME queries unchanged in the SAME order.`;
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      console.log(`[recheck] Attempt ${attempt}, ${originalQueries?.length || 0} locked queries`);
      const data = await callMoonshotAPI([
        { role: 'system', content: systemPrompt },
        { role: 'user', content: prompt },
      ]);
      ...
      if (!content) {
        lastError = new Error('Empty recheck content');
        throw lastError;
      }

      const raw = extractAndFixJSON(content);

      // 规范化：锁定原查询顺序，逐一对齐
      const score = Number.isFinite(Number(raw.score)) ? Math.max(0, Math.min(100, Math.round(Number(raw.score)))) : 0;
      const aiScores = Array.isArray(raw.query_match_scores) ? raw.query_match_scores : [];
      const validMatch = (m) => ['high', 'medium', 'low', 'fail'].includes(m) ? m : 'low';

      const queryMatchScores = (originalQueries || []).map((q, i) => {
        const item = aiScores[i] || {};
        return {
          query: q, // 强制使用原句，忽略 AI 可能的改写
          match: validMatch(item.match),
          reason: typeof item.reason === 'string' && item.reason.trim() ? item.reason.trim().slice(0, 200) : 'Re-evaluated against current page',
        };
      });

      // exposure_score：medium/high 占比
      const total = queryMatchScores.length;
      const good = queryMatchScores.filter(s => s.match === 'medium' || s.match === 'high').length;
      const exposureScore = total > 0 ? Math.round(good / total * 100) : 0;

      console.log(`[recheck] Success attempt ${attempt}: score=${score}, exposure=${exposureScore}%`);
      return {
        score,
        query_match_scores: queryMatchScores,
      ...
      lastError = error;
      console.error(`[recheck] Attempt ${attempt}/${MAX_ATTEMPTS} failed:`, error.message);
      if (attempt < MAX_ATTEMPTS) {
  ...
  throw new Error(`Recheck failed after ${MAX_ATTEMPTS} attempts. Last: ${lastError?.message}`);
}
