  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (content && content.trim().length > 0) {
        ...
        if (actualPrice && actualPrice !== '$0.00' && actualPrice !== '0') {
          normalized.diagnosis = (normalized.diagnosis || []).filter(d => {
            const issue = (d.issue || '').toLowerCase();
            const isFalsePriceClaim = issue.includes('$0.00') || issue.includes('price is listed as $0.00') || issue.includes('price.*0\\.00');
