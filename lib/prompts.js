...
/**
 * 构建复查提示词 — 锁定原7个买家查询原句，只重新评分，不重新生成查询
 * 保证 Day0 / Day14 / Day30 / Day60 的对比口径完全一致
 */
export function buildRecheckPrompt(productData, originalQueries, lang = 'en') {
  const title = sanitize(productData.title || 'Unknown', 200);
  const description = sanitize(productData.description || productData.pageText || '', 1200);
  const price = sanitize(productData.price || 'Unknown', 50);
  const queriesList = (originalQueries || [])
    .map((q, i) => `${i + 1}. ${sanitize(q, 200)}`)
    .join('\n');
  const langInstruction = (LANGUAGE_NAMES[lang] || LANGUAGE_NAMES.en) + ' (write all reasons in this language)';

  return `You are re-evaluating the SAME Shopify product page for AI search visibility. This is a follow-up recheck — the merchant may have changed the page since the first scan.

## The locked buyer queries (DO NOT change, reword, or regenerate these)
Evaluate the page against EXACTLY these ${originalQueries?.length || 0} queries, in this exact order:
${queriesList}

## Current page content (freshly scraped)
- Product: ${title}
- Price: ${price}
...
## Your task
For EACH locked query, re-grade how well the current page would let an AI assistant recommend this product when a real buyer asks that question. Use the same grading scale as the original report:
- "high": page directly and convincingly answers the query (strong, specific evidence)
- "medium": page partially addresses it (relevant but incomplete)
- "low": page only weakly relates to the query
- "fail": page provides essentially nothing for this query

CRITICAL rules:
- Do NOT invent new queries. Return the SAME queries in the SAME order, unchanged.
- Grade ONLY from the actual page content above. Never assume features, prices, claims, or policies that are not present.
- A $0.00 price on a Shopify page is normal (variant not selected / third-party selling) — never penalize it.
- Give a concrete, short reason citing what is present or missing on the CURRENT page.
- Then assign a new overall score (0-100 integer) reflecting the current page's overall AI visibility.
...
## Output JSON format (no markdown)
{
  "score": <integer 0-100>,
  "query_match_scores": [
    { "query": "<exact original query 1>", "match": "high|medium|low|fail", "reason": "<short evidence from current page>" }
  ]
}
