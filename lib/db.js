...
async function initDatabase() {
  ...
  // 每段 DDL 独立执行、独立容错：任何一段因旧表结构差异失败，
  // 都不影响其他表（尤其是新功能依赖的表）的创建。
  async function safeExec(label, sql) {
    try {
      await client.query(sql);
      return true;
    } catch (err) {
      console.warn(`[DB] DDL non-critical failure (${label}):`, err.message);
      return false;
    }
  }
  try {
    // 创建报告表
    await safeExec('reports', `
    ...
    // 创建验证码表
    await safeExec('verification_codes', `
    ...
    // 创建订阅用量追踪表
    await safeExec('subscription_usage', `
    ...
    // 创建创始用户（Founding Customers）表 — 前30名真实用户免费解锁
    await safeExec('founding_customers', `
      CREATE TABLE IF NOT EXISTS founding_customers (
        email         TEXT PRIMARY KEY,
        first_report_id VARCHAR(64),
        agreed_return BOOLEAN DEFAULT FALSE,
        agreed_feedback BOOLEAN DEFAULT FALSE,
        agreed_case   BOOLEAN DEFAULT FALSE,
        slot_number   INTEGER,
        created_at    TIMESTAMPTZ DEFAULT NOW()
      );
    `);

    // 报告复查快照（用原锁定的7个查询重跑后的结果）
    await safeExec('reports-recheck-cols', `
      ALTER TABLE reports ADD COLUMN IF NOT EXISTS recheck_snapshots JSONB;
      ALTER TABLE reports ADD COLUMN IF NOT EXISTS latest_recheck JSONB;
    `);

    console.log('[DB] Tables initialization finished');
    return true;
  } catch (err) {
...
/**
 * 独立兜底：只确保创始用户功能依赖的表和列存在。
 * 不经过 initDatabase 全流程，避免被其他无关 DDL 的失败拖累。
 */
async function ensureFoundingSchema() {
  const client = await getPool().connect();
  try {
    await client.query(`
      CREATE TABLE IF NOT EXISTS founding_customers (
 ...
 * 创始用户：查询当前已入组数量
 */
async function countFoundingCustomers() {
  const client = await getPool().connect();
  try {
    const result = await client.query(`SELECT COUNT(*)::int AS n FROM founding_customers`);
 ...
 * 创始用户：判断某邮箱是否已入组
 */
async function isFoundingCustomer(email) {
  if (!email) return false;
  const client = await getPool().connect();
  try {
    const result = await client.query(
      `SELECT 1 FROM founding_customers WHERE email = $1 LIMIT 1`,
      [email.toLowerCase()]
    );
    return result.rows.length > 0;
  } finally {
...
/**
 * 创始用户：登记入组（前30名）。返回入组结果和名额号。
 */
async function enrollFoundingCustomer(email, firstReportId, agreements = {}) {
  const client = await getPool().connect();
  try {
    const lower = email.toLowerCase();
    const existing = await client.query(
      `SELECT slot_number FROM founding_customers WHERE email = $1`,
      [lower]
    );
    if (existing.rows.length > 0) {
      return { enrolled: true, already: true, slot_number: existing.rows[0].slot_number };
    }
    const countRes = await client.query(`SELECT COUNT(*)::int AS n FROM founding_customers`);
    const slot = countRes.rows[0].n + 1;
    if (slot > 30) {
      return { enrolled: false, full: true, slot_number: null };
    }
    await client.query(`
      INSERT INTO founding_customers (email, first_report_id, agreed_return, agreed_feedback, agreed_case, slot_number)
      VALUES ($1, $2, $3, $4, $5, $6)
    `, [
      lower,
      firstReportId || null,
      !!agreements.agreed_return,
      !!agreements.agreed_feedback,
      !!agreements.agreed_case,
      slot,
    ]);
    return { enrolled: true, slot_number: slot };
  } finally {
    client.release();
  }
}
...
/**
 * 保存一次复查快照到原报告（追加到 recheck_snapshots，并更新 latest_recheck）
 */
async function addRecheckSnapshot(originalReportId, snapshot) {
  const client = await getPool().connect();
  try {
    const result = await client.query(`
      UPDATE reports
      SET recheck_snapshots = COALESCE(recheck_snapshots, '[]'::jsonb) || $2::jsonb,
          latest_recheck = $3::jsonb
      WHERE report_id = $1
      RETURNING report_id
    `, [
      originalReportId,
