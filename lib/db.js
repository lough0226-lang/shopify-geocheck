...
function resolveConnectionString() {
  // PG_CONN 优先：在 Zeabur 的 Vercel 兼容运行时里 DATABASE_URL 属于保留变量会被拦截，
  // 因此在 Zeabur 面板额外配置了一份同值的 PG_CONN。
  // 其余环境仍走标准的 DATABASE_URL 与 Postgres 服务自动注入变量。
  const direct = process.env.PG_CONN
  ...
  const host = process.env.POSTGRES_HOST || process.env.POSTGRESQL_HOST;
  if (host) {
    const user = process.env.POSTGRES_USERNAME || process.env.POSTGRES_USER || 'root';
    const pass = process.env.POSTGRES_PASSWORD || '';
    const port = process.env.POSTGRES_PORT || '5432';
    const db = process.env.POSTGRES_DATABASE || 'zeabur';
